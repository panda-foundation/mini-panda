...
func BinaryIR(c *Context, b *expression.Binary) ir.Value {
	...
	case token.Assign:
		if ast_types.IsPointer(b.Left.Type()) && ast_types.IsArray(b.Right.Type()) {
			var gep instruction.Instruction = instruction.NewGetElementPtr(TypeIR(b.Right.Type()), v2, constant.NewInt(ir_types.I32, 0), constant.NewInt(ir_types.I32, 0))
...
func BinaryConstIR(p *Program, b *expression.Binary) constant.Constant {
